...
from apps.armory.services import RangeService
...
def listar_armas(
    ...
    return templates.TemplateResponse("armory/index.html", {
        "request": request,
        "guns": guns,
        "stats": stats,
        "user": user
...
def detalhe_arma(
    ...
    return templates.TemplateResponse("armory/detail.html", {
        "request": request,
        "gun": gun,
        "user": user
    })
...
def adicionar_acessorio(
    ...
    return RedirectResponse(url=f"/armory/{gun_id}", status_code=303)
